...
def _linprog_ip(x, c, A_ub, b_ub, A_eq, b_eq,
    ...
    for step in range(1, max_steps+1):
        ...
        if A_eq is not None:
            ...
            # TODO check that this is the right step if we're already feasible; maybe simpler
        ...
        lamb = v.t() @ dxw   # TODO: is this the correct stopping criterion for equality-constrained Newton??
...
# TODO: change inverse_linprog argument semantics to the following convention:
#   None => uniform prior, i.e. no prior at all (alternative: have inverse_opt.NO_PRIOR constant)
#   Prior object => specific soft prior on the parameters, where Prior is some type defined in inverse_opt
#   Array value => Hard prior on the parameters, i.e. no learning is done regardless of learning rate
...
def inverse_linprog(x_target, c, A_ub, b_ub, A_eq=None, b_eq=None,
    ...
    for step in range(1, max_steps+1):
#         print("step ", step)
#         print("check eps in outer loop: ",eps_range(step-1))
